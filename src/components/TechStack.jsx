...
const accentStyles = {
  cyan: {
    number: "border-cyan-300/30 bg-cyan-300/[0.04] text-cyan-200",
    dot: "bg-cyan-300 shadow-[0_0_14px_rgba(103,232,249,0.55)]",
    line: "from-cyan-300/55",
    hover: "hover:border-cyan-300/25",
  },
  blue: {
    number: "border-blue-400/30 bg-blue-400/[0.04] text-blue-200",
    dot: "bg-blue-400 shadow-[0_0_14px_rgba(96,165,250,0.55)]",
    line: "from-blue-400/55",
    hover: "hover:border-blue-400/25",
  },
  violet: {
    number: "border-violet-400/30 bg-violet-400/[0.04] text-violet-200",
    dot: "bg-violet-400 shadow-[0_0_14px_rgba(167,139,250,0.55)]",
    line: "from-violet-400/55",
    hover: "hover:border-violet-400/25",
  },
  emerald: {
    number: "border-emerald-400/30 bg-emerald-400/[0.04] text-emerald-200",
    dot: "bg-emerald-400 shadow-[0_0_14px_rgba(52,211,153,0.55)]",
    line: "from-emerald-400/55",
    hover: "hover:border-emerald-400/25",
  },
  rose: {
    number: "border-rose-400/30 bg-rose-400/[0.04] text-rose-200",
    dot: "bg-rose-400 shadow-[0_0_14px_rgba(251,113,133,0.5)]",
    line: "from-rose-400/55",
    ...
    >
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-start gap-4">
          <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-xs font-semibold ${accent.number}`}>
            {String(index + 1).padStart(2, "0")}
          </span>
          <div>
            <div className="flex items-center gap-3">
              <span className={`h-1.5 w-1.5 rounded-full ${accent.dot}`} />
              <h3 className="text-xl font-semibold tracking-[-0.03em] text-white sm:text-2xl">{row.label}</h3>
            </div>
        ...
        <span className="hidden text-[10px] uppercase tracking-[0.18em] text-slate-700 sm:block">
          {row.direction === "left" ? "Slow drift left" : "Slow drift right"}
        </span>
      </div>
      ...
      <div className="relative overflow-hidden py-1">
        ...
        <motion.div
          ...
          className="flex w-max gap-2.5 px-2"
        >
          {repeatedTools.map(([name, logo], toolIndex) => (
            <motion.div
              ...
              className={`flex h-13 min-w-max items-center gap-3 rounded-full border border-white/[0.08] bg-white/[0.025] px-4 backdrop-blur-sm transition ${accent.hover} hover:bg-white/[0.05]`}
            >
